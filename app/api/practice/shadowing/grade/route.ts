...
const PASS_PROSODY_THRESHOLD = 0.75;
...
export async function POST(req: Request) {
  ...
  if (!targetText || !from || !to) {
    return NextResponse.json({ error: "Missing targetText/from/to" }, { status: 400 });
  }
  ...
  try {
    ...
    console.error("[/api/practice/shadowing/grade] transcode failed:", err);
  ...
  const prompt = renderPrompt("shadowing_grade", {
  ...
  try {
    const completion = await ai.chat.completions.create({
      ...
      temperature: SAMPLING.shadowing_grade.temperature,
      max_completion_tokens: SAMPLING.shadowing_grade.max_completion_tokens,
    ...
    const wordsMatch = parsed.wordsMatch === true;
    const prosodyScore =
      typeof parsed.prosodyScore === "number" ? Math.max(0, Math.min(1, parsed.prosodyScore)) : 0;
    const transcript = typeof parsed.transcript === "string" ? parsed.transcript.trim() : "";
    const feedback = typeof parsed.feedback === "string" ? parsed.feedback.trim() : "";
    const passed = wordsMatch && prosodyScore >= PASS_PROSODY_THRESHOLD;

    return NextResponse.json({ passed, wordsMatch, prosodyScore, transcript, feedback });
  } catch (err) {
    console.error("[/api/practice/shadowing/grade] failed:", err);
    return NextResponse.json(
